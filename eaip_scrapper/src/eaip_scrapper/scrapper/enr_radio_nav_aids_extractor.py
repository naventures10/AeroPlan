...
class ENRRadioNavAidsExtractor(BaseENRExtractor):
    ...
    def __init__(self, active_eaip_url, session=None, output_file="enr_4_1_radio_nav_aids.json"):
        ...
        # airac_resolver is now handled by base_enr_extractor
        # self.resolver = AIRACResolver(self.home_url, session=self.session)
        # self.active_eaip_url = None # Handled by base_enr_extractor

    # Removed _resolve_enr_page_url and _fetch_and_parse as they are now handled by base_enr_extractor
    ...
    # Removed extract_and_save as it's now handled by base_enr_extractor's run() method

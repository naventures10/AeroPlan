...
from eaip_scrapper.validation.schemas.ingest.aerodrome import (
    GeographicalData,
    Helipad,
    NavAid,
    Obstacle,
...
class AIPSchemaMapper:
    ...
    def map_obstacles_table(self, grid):
        ...
        try:
            # Enforce schema validation (e.g., coordinate regex)
            for obs in obstacles_data:
                Obstacle(**obs)
        except ValidationError as e:
            print("[!] CRITICAL: Obstacle data failed schema validation!")
            print(e)
            sys.exit(1)
    ...
    def map_aerodrome_geo_table(self, grid):
        ...
        try:
            GeographicalData(**geo_data)
        except ValidationError as e:
            print("[!] CRITICAL: Geographical data failed schema validation!")
            print(e)
            sys.exit(1)
    ...
    def map_runway_physical_characteristics(self, grid):
        ...
        try:
            for rwy in runways:
                RunwayCharacteristic(**rwy)
        except ValidationError as e:
            print("[!] CRITICAL: Runway physical characteristics failed schema validation!")
            print(e)
            sys.exit(1)
    ...
    def map_helicopter_landing_area(self, grid):
        ...
        try:
            Helipad(**heli_landing_area)
        except ValidationError as e:
            print("[!] CRITICAL: Helipad data failed schema validation!")
            print(e)
            sys.exit(1)
    ...
    def map_radio_navigation_aids(self, grid):
        ...
        try:
            for aid in radio_aids:
                NavAid(**aid)
        except ValidationError as e:
            print("[!] CRITICAL: NavAid data failed schema validation!")
            print(e)
            sys.exit(1)

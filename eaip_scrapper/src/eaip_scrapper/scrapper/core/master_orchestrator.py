...
class MasterOrchestrator:
    ...
    def _process_airport(self, entry):
        ...
        Thread-safe: each call uses its own live_table_extractor and chart_extractor
...
if __name__ == "__main__":
    ...
    print("[*] Resolving Active AIRAC Cycle (Standalone Run)")

    session = requests.Session()
    session.verify = False

    resolver = AIRACResolver(HOME_URL, session=session)
    active_eaip_url = resolver.get_current_eaip_url()

    if not active_eaip_url:
        print("[!] Critical Failure: Could not resolve a valid eAIP target URL.")
        exit(1)

    print(f"[+] Target: {active_eaip_url}\n")

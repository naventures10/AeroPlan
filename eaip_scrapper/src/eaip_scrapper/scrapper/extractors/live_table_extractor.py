...
class LiveTableExtractor:
    ...
    def _extract_from_tables(self, tables, section_id, mode):
        ...
        for i, table in enumerate(tables):
            ...
            # Use regex for exact section boundary matching (e.g. AD 2.1 won't match AD 2.10)
            import re

            if (
                re.search(rf"\b{re.escape(section_id)}\b", table_text)
                and get_primary_row_count(table) <= 3
            ):
                target_index = i

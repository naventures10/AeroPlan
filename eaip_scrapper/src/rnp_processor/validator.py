...
import math
import re
from .utils import clean_text, is_valid_coord, INDIA_LAT_MIN, INDIA_LAT_MAX, INDIA_LON_MIN, INDIA_LON_MAX
            ...
            if ident and ident.upper() in wpt_lookup:
                ...
                if prev_coord:
                    ...
                    if dist_deg > 8.3:  # ~500 NM
                        issues.append(
                            f"Massive jump ({dist_deg:.1f}°) between legs "
                            f"near {ident}"
                        )
                prev_coord = coord

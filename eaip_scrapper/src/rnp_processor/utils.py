...
def parse_coordinate(coord_str):
    ...
    compact_lon = re.search(
        r'(\d{2,3})(\d{2})(\d{2}(?:\.\d+)?)\s*([EW])',
        coord_str, re.IGNORECASE
    )
    if compact_lat and compact_lon:
        lat_dir = compact_lat.group(4).upper()
        lon_dir = compact_lon.group(4).upper()
        lat_dd = dms_to_dd(compact_lat.group(1), compact_lat.group(2),
                           compact_lat.group(3), lat_dir)
        lon_dd = dms_to_dd(compact_lon.group(1), compact_lon.group(2),
                           compact_lon.group(3), lon_dir)
    ...
    if lat_with_dir:
        ...
        lon_no_dir = re.search(
            r'(\d{1,3})\s*[:°]\s*(\d{1,2})\s*[:\'′]\s*(\d{1,2}(?:\.\d+)?)',
            remaining
        )
        if lon_no_dir:
            lat_dir = lat_with_dir.group(4).upper()
            lat_dd = round(dms_to_dd(
                lat_with_dir.group(1), lat_with_dir.group(2),
                lat_with_dir.group(3), lat_dir
            ), 7)
            # Default to East for Indian airspace
            lon_dd = round(dms_to_dd(
                lon_no_dir.group(1), lon_no_dir.group(2),
                lon_no_dir.group(3), 'E'
            ), 7)
            if is_valid_coord(lat_dd, lon_dd):
                return lat_dd, lon_dd

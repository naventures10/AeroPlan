...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let aspect = uniforms.resolution.x / max(uniforms.resolution.y, 0.001);
    let uv = (in.uv * 2.0 - 1.0) * vec2<f32>(aspect, 1.0);
    let time = uniforms.metadata.x;
    
    let sf = 2.0 / max(min(uniforms.resolution.x, uniforms.resolution.y), 0.001);
    ...
    let grid_color = vec3<f32>(0.0, 0.4, 0.8) * pulse;

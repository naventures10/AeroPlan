...
export default function MapDisclaimerBanner() {
  ...
  if (isMinimized) {
    return (
      <HoverTooltip<HTMLButtonElement> content="Aeronautical Disclaimer">
        {({ ref, interestfor, className }) => (
          <button
            ...
            onClick={() => {
              ...
              // Give user 5 seconds of idle view before auto-minimizing again if they interact
              scheduleAutoMinimize();
            }}
            className={`glass-morphism pointer-events-auto z-40 flex items-center justify-center p-2.5 rounded-xl text-accent-cyan hover:bg-surface-container-high transition-all duration-300 shadow-md ${className} ${
              isMobile ? 'fixed bottom-16 right-3' : 'absolute bottom-6 right-6'
            }`}
  ...
  return (
    <div
      className={`glass-morphism pointer-events-auto z-40 flex items-center gap-3 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl transition-all duration-300 shadow-md text-on-surface ${
        isMobile
          ? 'fixed bottom-16 left-3 right-3 max-w-[calc(100vw-1.5rem)] text-xs'

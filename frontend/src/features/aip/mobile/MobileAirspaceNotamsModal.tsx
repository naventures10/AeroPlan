import { X, Search, Filter } from 'lucide-react';
import { motion, AnimatePresence, useDragControls } from 'framer-motion';
import { useEffect, useState, memo } from 'react';
import { useMapStore } from '../../../store/useMapStore';
import { fetchAirspaceNotams } from '../../../api/client';
import type { NotamData } from '../../../types';
import { sanitizeNotamDescription } from '../../../utils/sanitize';
import './MobileAirspaceNotamsModal.css';

export const MobileAirspaceNotamsModal = memo(function MobileAirspaceNotamsModal() {
  const isAirspaceNotamsModalOpen = useMapStore((s) => s.isAirspaceNotamsModalOpen);
  const setAirspaceNotamsModalOpen = useMapStore((s) => s.setAirspaceNotamsModalOpen);
  const viewMode = useMapStore((s) => s.viewMode);
  const pitch = useMapStore((s) => s.viewState.pitch);
  const [notams, setNotams] = useState<NotamData[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFir, setSelectedFir] = useState<string>('All');
  const dragControls = useDragControls();

  useEffect(() => {
    if (isAirspaceNotamsModalOpen) {
      const loadNotams = async () => {
        setIsLoading(true);
        try {
          const data = await fetchAirspaceNotams();
          setNotams(data);
        } catch (error) {
          console.error('Failed to load airspace NOTAMs:', error);
          setNotams([]);
        } finally {
          setIsLoading(false);
        }
      };
      loadNotams();
    }
  }, [isAirspaceNotamsModalOpen]);

  if (!isAirspaceNotamsModalOpen || viewMode !== 'ENROUTE' || pitch > 0) return null;

  const onClose = () => setAirspaceNotamsModalOpen(false);

  const formatDate = (dateStr?: string | null) => {
    if (!dateStr) return 'PERM / UFN';
    return new Date(dateStr).toLocaleString('en-GB', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
      hour: '2-digit',
      minute: '2-digit',
      timeZone: 'UTC',
      timeZoneName: 'short',
    });
  };

  const uniqueFirs = Array.from(
    new Set(notams.flatMap((n) => [n.fir, n.combined_fir]).filter(Boolean) as string[]),
  ).sort();

  // fallow-ignore-next-line complexity
  const filteredNotams = notams.filter((notam) => {
    const q = searchQuery.toLowerCase();
    const matchesSearch =
      notam.notam_id.toLowerCase().includes(q) ||
      (notam.description && notam.description.toLowerCase().includes(q)) ||
      (notam.fir && notam.fir.toLowerCase().includes(q)) ||
      (notam.combined_fir && notam.combined_fir.toLowerCase().includes(q));

    const matchesFir =
      selectedFir === 'All' || notam.fir === selectedFir || notam.combined_fir === selectedFir;

    return matchesSearch && matchesFir;
  });

  return (
    <AnimatePresence>
      {isAirspaceNotamsModalOpen && (
        <>
          {/* Backdrop */}
          <motion.div
            key="mobile-notams-backdrop"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="aip-mobile-notams-backdrop"
          />

          {/* Drawer */}
          <motion.div
            key="mobile-notams-drawer"
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            drag="y"
            dragControls={dragControls}
            dragListener={false}
            dragConstraints={{ top: 0 }}
            dragElastic={{ top: 0.05, bottom: 0.95 }}
            onDragEnd={(_e, info) => {
              if (info.offset.y > 120 || info.velocity.y > 350) {
                onClose();
              }
            }}
            className="aip-mobile-notams-drawer"
            onClick={(e) => e.stopPropagation()}
            onWheel={(e) => e.stopPropagation()}
          >
            {/* Drag Zone and Handle */}
            <div
              className="aip-mobile-notams-drag-zone"
              onPointerDown={(e) => dragControls.start(e)}
              style={{ touchAction: 'none' }}
            >
              <div className="aip-mobile-notams-drag-handle" />
            </div>

            {/* Header */}
            <div className="aip-mobile-notams-header">
              <h2 className="aip-mobile-notams-title">Enroute Notams</h2>
              <button
                onClick={onClose}
                className="aip-mobile-notams-close-btn"
                aria-label="Close modal"
              >
                <X size={20} />
              </button>
            </div>

            {/* Sticky Search & Filter Controls */}
            <div className="aip-mobile-notams-controls">
              <div className="aip-mobile-notams-search-wrapper">
                <Search size={18} className="aip-mobile-notams-search-icon" />
                <input
                  type="text"
                  placeholder="Search by ID, FIR, or keyword..."
                  className="aip-mobile-notams-search-input"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
              <div className="aip-mobile-notams-fir-wrapper">
                <Filter size={16} className="aip-mobile-notams-fir-icon" />
                <select
                  className="aip-mobile-notams-fir-select"
                  value={selectedFir}
                  onChange={(e) => setSelectedFir(e.target.value)}
                  aria-label="Filter by FIR"
                >
                  <option value="All">All FIRs</option>
                  {uniqueFirs.map((fir) => (
                    <option key={fir} value={fir}>
                      {fir}
                    </option>
                  ))}
                </select>
              </div>
            </div>

            {/* Scrollable Content */}
            <div className="aip-mobile-notams-content aip-scrollbar">
              {isLoading ? (
                <div className="aip-mobile-notams-loading">
                  <div className="aip-mobile-notams-spinner" />
                  <span>Loading Enroute Notams...</span>
                </div>
              ) : filteredNotams.length === 0 ? (
                <div className="aip-mobile-notams-empty">
                  <span>No matching NOTAMs found.</span>
                </div>
              ) : (
                <div className="aip-mobile-notams-grid">
                  {filteredNotams.map((notam) => (
                    <div key={notam.notam_id} className="aip-mobile-notam-card">
                      <div className="aip-mobile-notam-header">
                        <div className="aip-mobile-notam-id">{notam.notam_id}</div>
                        <div className="aip-mobile-notam-badges">
                          {notam.fir && (
                            <span className="aip-mobile-notam-badge fir">FIR: {notam.fir}</span>
                          )}
                          {notam.combined_fir && (
                            <span className="aip-mobile-notam-badge fir">
                              FIR: {notam.combined_fir}
                            </span>
                          )}
                        </div>
                      </div>

                      <div className="aip-mobile-notam-dates">
                        <span>Valid: {formatDate(notam.valid_from)}</span>
                        <span>—</span>
                        <span>{notam.is_permanent ? 'PERM' : formatDate(notam.valid_to)}</span>
                      </div>

                      <div className="aip-mobile-notam-desc">
                        {sanitizeNotamDescription(notam.description)}
                      </div>
                    </div>
                  ))}
                </div>
              )}
            </div>
          </motion.div>
        </>
      )}
    </AnimatePresence>
  );
});

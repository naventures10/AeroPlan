...
export default function TerminalDashboard({ icaoCode }: { icaoCode: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/weather/${icaoCode}`, { signal })
      ...
      .then((w) => {
        if (!signal.aborted && w) setWeather(w);
      })
      .catch(() => {})
      .finally(maybeFinish);
    ...
    fetch(`/api/daylight/${icaoCode}?date=${todayStr}`, { signal })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (!signal.aborted) setDaylight(d?.records?.[0] || null);
      })
      .catch(() => {})
      .finally(maybeFinish);
              ...
              {activeTab === 'METAR' && (
                ...
                >
                  ...
                  <div className="bg-zinc-950 p-6 rounded-2xl border border-zinc-800/50 grow">
                    {weather?.metar ? (
                      <p className="text-lg text-emerald-400 font-mono leading-relaxed">
                        {weather.metar}
                      </p>
                    ) : (

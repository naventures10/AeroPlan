...
describe('createAirspaceLayers', () => {
  it('creates geometry and metadata MVT layers', () => {
    ...
    const longFeature = { properties: { airspace_type: 'CTA_LOWER', name: 'THIS IS A VERY LONG NAME THAT EXCEEDS LIMITS' } };
    ...
    expect(highlightLayers[1].props.getBackgroundColor(highlightFeature)).toEqual([255, 255, 0, 40]);
    expect(highlightLayers[1].props.getBorderColor(highlightFeature)).toEqual([255, 255, 0, 255]);

    // Check non-highlight borders
    expect(highlightLayers[1].props.getBorderColor({ properties: { airspace_type: 'FIR', name: 'FIR' } })).toEqual([255, 165, 0, 255]);
    // Without name it's invisible
    expect(highlightLayers[1].props.getBorderColor({ properties: { airspace_type: 'FIR' } })).toEqual([0, 0, 0, 0]);

...
describe('API Client Functions', () => {
  ...
  it('should throw ApiError with correct properties on failed get()', async () => {
    ...
    try {
      ...
      expect(err.path).toBe('/aerodromes');
  ...
  it('should return [] on chart fetch failure', async () => {
    (global.fetch as any).mockResolvedValueOnce({ ok: false, status: 500, text: () => Promise.resolve('') });
    const result = await fetchCharts('VOBM');
    expect(result).toEqual([]);
  });
  ...
  it('should fetchWeather returning data', async () => {
    (global.fetch as any).mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ metar: 'METAR ...' }) });
    const res = await fetchWeather('VOBM');
    expect(res).toEqual({ metar: 'METAR ...' });
  });

  it('should fetchNotams returning data', async () => {
    (global.fetch as any).mockResolvedValueOnce({ ok: true, json: () => Promise.resolve([{ notam_id: 'A123' }]) });
  ...
  it('should fetchAtsRouteLabels returning data', async () => {
    (global.fetch as any).mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ type: 'FeatureCollection' }) });
  ...
  it('should fetchNavaidDetails returning data', async () => {
    (global.fetch as any).mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ ident: 'BIA' }) });

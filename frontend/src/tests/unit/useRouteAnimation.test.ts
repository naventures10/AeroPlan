...
describe('useRouteAnimation', () => {
  ...
  it('updates currentTime on requestAnimationFrame when playing', () => {
    useMapStore.setState({
  ...
  it('fetches and builds animation when routes are selected', async () => {
    ...
    (api.fetchAtsRouteDetails as any).mockResolvedValue({
  ...
  it('handles API failure gracefully', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith(
        'Failed to build ATS routes animation',
        expect.any(Error),
      );
    });
  ...
  it('isAtsRendered state updates immediately on and delayed off', () => {
    const { result } = renderHook(() => useRouteAnimation());
    expect(result.current.isAtsRendered).toBe(false);

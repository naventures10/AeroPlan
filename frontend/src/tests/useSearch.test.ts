...
describe('useSearch', () => {
  ...
  it('should perform search and update suggestions', async () => {
    (client.searchAll as any).mockResolvedValue([
      { id: '1', name: 'Test', type: 'AERODROME' }
    ]);
    const { result } = renderHook(() => useSearch());
  ...
  it('should handle clearing the search input (empty string)', async () => {
    const { result } = renderHook(() => useSearch());
  ...
  it('should select an aerodrome feature correctly', async () => {
    ...
    await act(async () => {
      await result.current.handleGlobalSearchSelect({
        id: 'VAAU',
        name: 'VAAU',
        type: 'AERODROME',
        center: [70, 20]
  ...
  it('should handle selection without geometry/center', async () => {
    const { result } = renderHook(() => useSearch());
    await act(async () => {
      await result.current.handleGlobalSearchSelect({
        id: 'VAAU',
  ...
  it('should handle ATS_ROUTE selection', async () => {
    const storeState = useMapStore.getState();
  ...
  it('should handle WAYPOINT selection', async () => {
    const storeState = useMapStore.getState();
    const flyToSpy = vi.spyOn(storeState, 'flyToLocation');
    const toggleLayerSpy = vi.spyOn(storeState, 'toggleLayer');
    ...
    await act(async () => {
      await result.current.handleGlobalSearchSelect(feature as any);
    });
    ...
    expect(setSelectedFeatureSpy).toHaveBeenCalledWith({ type: 'WAYPOINT', data: feature.properties });
  });

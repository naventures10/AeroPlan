...
describe('createNavaidLayer', () => {
  it('creates MVT navaid layer with proper styling', () => {
    ...
    expect(highlightLayers[0].props.getIconColor({ properties: { ident: 'TEST' } })).toEqual([0, 255, 255, 255]);
    expect(highlightLayers[0].props.getIconColor({ properties: { ident: 'OTHER' } })).toEqual([52, 211, 153, 255]);

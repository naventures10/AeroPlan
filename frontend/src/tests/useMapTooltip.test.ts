...
describe('useMapTooltip', () => {
  ...
  it('handles aerodromes layer', () => {
    useMapStore.setState({ activeAerodromeMetadata: null, activeLayers: {} as any, selectedRouteIds: [] });
    const { result } = renderHook(() => useMapTooltip({ current: null }));

    const tooltip = result.current({
      object: { properties: { icao_code: 'VAAU', aerodrome_name: 'Test', elevation: '100', communications: '[{"service":"TWR","frequency":"118.1"}]' } },
  ...
  it('handles ats routes layer (geom layer)', () => {
    useMapStore.setState({ activeAerodromeMetadata: null, activeLayers: { atsRoutes: true } as any, selectedRouteIds: [] });
    const { result } = renderHook(() => useMapTooltip({ current: null }));

    const tooltip = result.current({
      object: { properties: { route_id: 'L333', route_type: 'RNAV', lower_limit: 'FL150', upper_limit: 'FL400', distance_nm: '100' } },
    ...
    const mockMapRef = {
      current: {
        getMap: vi.fn(() => ({
          ...
          queryRenderedFeatures: vi.fn((point, options) => {
        ...
        getMap: vi.fn(() => ({
          ...
          queryRenderedFeatures: vi.fn((point, options) => {
            if (options.layers?.includes('mvt-polygons')) {
              return [{
                layer: { id: 'mvt-polygons' },
                properties: { feature_type: 'RESTRICTED', name: 'R1', class: '', lower_limit: 'SFC', upper_limit: 'FL100', remarks: 'test', type: 'RESTRICTED' }
              }];
            }
            return [];
          })
  ...
  it('handles maplibre map hits for aerodrome features with metadata', () => {
    ...
    useMapStore.setState({ activeAerodromeMetadata: mockMetadata as any, activeLayers: {} as any, selectedRouteIds: [] });
    const mockMapRef = {
      current: {
        getMap: vi.fn(() => ({
          getStyle: vi.fn(() => ({
          ...
          queryRenderedFeatures: vi.fn((point, options) => {
            if (options.layers?.includes('mvt-points')) {
              return [
                ...
                {
                  layer: { id: 'mvt-points' },
                  properties: { feature_type: 'RUNWAY_THRESHOLD', name: '09', type: 'RUNWAY_THRESHOLD' } // category
                }
              ];
            }
            return [];

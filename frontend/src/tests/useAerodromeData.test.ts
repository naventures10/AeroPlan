...
describe('useAerodromeData', () => {
  ...
  it('should handle section selection correctly', async () => {
    (client.fetchAerodromes as any).mockResolvedValue([]);
    (client.fetchAerodromeSection as any).mockResolvedValue({ title: 'T', data_type: 'txt', data: 'd' });
  ...
  it('should do nothing on section selection if activeAirport is null', async () => {
    (client.fetchAerodromes as any).mockResolvedValue([]);
    let result: any;
    await act(async () => {
      const rendered = renderHook(() => useAerodromeData());

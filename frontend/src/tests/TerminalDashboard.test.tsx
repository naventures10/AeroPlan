...
describe('TerminalDashboard', () => {
  ...
  it('renders with data and switches tabs', async () => {
    ...
    let container: any;
    await act(async () => {
      container = render(<TerminalDashboard icaoCode="VAAU" />);
    });
    ...
    const collapseButton = document.querySelector('button[title=""]');
    if(collapseButton) {
      act(() => {
  ...
  it('renders nothing when no icaoCode is provided', async () => {
    render(<TerminalDashboard icaoCode="" />);

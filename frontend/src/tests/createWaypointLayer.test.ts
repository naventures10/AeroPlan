...
describe('createWaypointLayer', () => {
  it('creates MVT waypoint layer with proper styling', () => {
    ...
    expect(highlightLayers[0].props.getIconColor({ properties: { waypoint_name: 'TEST' } })).toEqual([0, 255, 255, 255]);
    expect(highlightLayers[0].props.getIconSize({ properties: { waypoint_name: 'TEST' } })).toBe(16);
    ...
    expect(ctx.setSelectedFeature).toHaveBeenCalledWith({ type: 'WAYPOINT', data: { waypoint_name: 'W1' } });
  });

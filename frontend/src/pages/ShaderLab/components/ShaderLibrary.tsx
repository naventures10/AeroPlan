...
export const ShaderLibrary = ({ shaders, activeShaderId, show, onSelect }: ShaderLibraryProps) => (
  <div
    className={`pointer-events-auto max-w-5xl rounded-3xl border border-white/8 bg-black/40 p-3 backdrop-blur-xl shadow-2xl overflow-hidden transition duration-500 [transition-property:opacity,transform,backdrop-filter,-webkit-backdrop-filter] ${show ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}
  >
    {show && (
      <>

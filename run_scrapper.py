...
from src.scrapper.ENROtherRegulatedAirspaceExtractor import ENROtherRegulatedAirspaceExtractor
from src.scrapper.ENRProhibitedAreasExtractor import ENRProhibitedAreasExtractor
from src.scrapper.ENRMilitaryExerciseAreasExtractor import ENRMilitaryExerciseAreasExtractor
...
if __name__ == "__main__":
    ...
    enr_tasks = [
        (ENRRadioNavAidsExtractor(active_eaip_url, session=master_session, output_file="output/enr_4_1_radio_nav_aids.json").extract_and_save, "ENR 4.1"),
        (ENRAirspaceExtractor(active_eaip_url, session=master_session, output_file="output/enr_2_1_airspace.json").extract_and_save, "ENR 2.1"),
        (ENROtherRegulatedAirspaceExtractor(active_eaip_url, session=master_session, output_file="output/enr_2_2_other_regulated_airspace.json").extract_and_save, "ENR 2.2"),
        (ENRProhibitedAreasExtractor(active_eaip_url, session=master_session, output_file="output/enr_5_1_prohibited_restricted_danger.json").extract_and_save, "ENR 5.1"),
        (ENRMilitaryExerciseAreasExtractor(active_eaip_url, session=master_session, output_file="output/enr_5_2_military_exercise_adiz.json").extract_and_save, "ENR 5.2"),
        (ENRHelicopterRoutesExtractor(active_eaip_url, session=master_session, output_file="output/enr_3_3_1_helicopter_routes.json").extract_and_save, "ENR 3.3.1"),
        (ENRUPRZonesExtractor(active_eaip_url, session=master_session, output_file="output/enr_3_3_2_upr_zones.json").extract_and_save, "ENR 3.3.2"),
        (ENREnRouteChartsExtractor(active_eaip_url, session=master_session, output_file="output/enr_6_en_route_charts.json").extract_and_save, "ENR 6"),
        (ENRRoutesExtractor(
            active_eaip_url, 
            session=master_session, 
            output_file_31="output/enr_3_1_conventional_routes.json", 
            output_file_32="output/enr_3_2_rnav_routes.json"
        ).extract_and_save, "ENR 3.1 & 3.2"),
        (ENRSignificantPointsExtractor(active_eaip_url, session=master_session, output_file="output/enr_4_4_significant_points.json").extract_and_save, "ENR 4.4")
    ]

...
class AIPSchemaMapper:
    ...
    def map_runway_physical_characteristics(self, grid):
        ...
        # Strict regex: 01-36 with optional L, C, or R (e.g., "05", "23", "09L")
        designator_pattern = re.compile(r"^(0[1-9]|[12][0-9]|3[0-6])\s*[LCR]?$")
        ...
        for row in grid:
            ...
            if len(row) > 0 and designator_pattern.match(row[0].strip()):
                
                # Directly map the 14 columns across the horizontally stitched grid
                rwy_data = {
                    "designation": clean(row[0]),
    ...
    def map_declared_distances_table(self, grid):
        ...
        # Strict regex: 01-36 with optional L, C, or R (e.g., "08", "26")
        designator_pattern = re.compile(r"^(0[1-9]|[12][0-9]|3[0-6])\s*[LCR]?$")
    ...
    def map_radio_navigation_aids(self, grid):
        ...
        for row in grid:
            ...
            if not col0_text or col0_text.isdigit() or "TYPE OF" in col0_text or "GEOGRAPHICAL" in col0_text:
                continue
    ...
    def map_local_regulations(self, grid):
        ...
        for row in grid:
            ...
            # Skip title headers
            if len(unique_cells) == 1 and any(h == unique_cells[0].upper() for h in ignore_headers):
                continue
    ...
    def map_noise_abatement_procedures(self, grid):
        ...
        # Standard headers to filter out if they bleed into the grid
        ignore_headers = ["AD 2.21", "NOISE ABATEMENT PROCEDURES", "VAAH", "VAAM", "VABB", "VIDP"]
    ...
    def map_flight_procedures(self, grid):
        ...
        ignore_headers = ["AD 2.22", "FLIGHT PROCEDURES", "VAAH", "VAAM", "VABB", "VIDP"]
    ...
    def map_additional_information(self, grid):
        ...
        # Standard headers to filter out
        ignore_headers = ["AD 2.23", "ADDITIONAL INFORMATION", "VAAH", "VAAM", "VABB", "VIDP"]

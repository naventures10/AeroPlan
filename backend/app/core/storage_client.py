...
class UnifiedStorageClient:
    ...
    def generate_presigned_url(self, s3_key: str, expiration: int = 7200) -> str:
        ...
        if self.is_gcs:
            ...
            # Fallback to public URL if signing completely fails
            logger.error("All GCS signing attempts failed, falling back to public URL.")
            return f"https://storage.googleapis.com/{self.bucket_name}/{s3_key}"
        else:

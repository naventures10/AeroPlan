...
@pytest.mark.asyncio
async def test_get_aerodrome_metadata_row_none_coverage(api_client: AsyncClient, db_session) -> None:
    from unittest.mock import MagicMock
    async def mock_execute(*args, **kwargs):
        mock_result = MagicMock()
        mock_result.fetchone.return_value = None
...
@pytest.mark.asyncio
async def test_get_aerodrome_metadata_row_true_but_row0_falsy(api_client: AsyncClient, db_session) -> None:
    from unittest.mock import MagicMock
...
@pytest.mark.asyncio
async def test_get_aerodrome_metadata_row_empty_sequence(api_client: AsyncClient, db_session) -> None:
    from unittest.mock import MagicMock
    class EmptyRow:
        def __bool__(self):
            return True
